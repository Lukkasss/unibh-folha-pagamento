...
public class Comissionado extends Funcionarios{
    ...
    public Comissionado(String matricula, String nome, String cpf, Double valorVendas, Double comissao, Double vr){

        super.setSalario(valorVendas, comissao);
    ...
    public Comissionado(String matricula, String nome, String cpf, Double valorVendas, Double comissao, Double vr, ArrayList<Dependentes> dep){
    ...
    public String montaDependentes() {
        ...
        if(this.getDependentes().size() != 0){
            ...
            for(int i = 0; i < super.getDependentes().size(); i++)
                dados += super.getDependentes().get(i).toString();
            return dados;
    ...
    public String montarRelatorio(){
        return String.format("Matrícula: %s\nNome: %s\nCPF: %s\nAlíquota do INSS: %.0f%%\nDesconto INSS: %.2f\nAlíquota IRRF: %.1f%%\nDesconto IRRF: %.2f\nVale-Refeição: %.2f\nDesconto Vale-Refeição: %.2f\nDesconto Vale-Transporte: %.2f\nSalário Bruto: %.2f\nSalário Líquido: %.2f\nQuantidade de Dependentes Cadastrados: %d\nLista de Dependentes: %s",
                ...
                this.calcularSalario("l"), super.getDependentes().size(), montaDependentes());
    }
}
